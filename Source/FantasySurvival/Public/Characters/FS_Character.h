...
protected:
	...
	UPROPERTY(Transient)
	UFS_AttributeSet_Stats* StatsSet;
	...
	// TEMP for vertical slice: Abilties we grant on possess (Server)
	UPROPERTY(EditDefaultsOnly, Category = "GAS|Startup")
	TArray<TSubclassOf<UGameplayAbility>> StartupAbilities;

	// Effects we apply on possess and keep running (e.g., Stamina Regen)
	UPROPERTY(EditDefaultsOnly, Category = "GAS|Startup")
	TArray<TSubclassOf<class UGameplayEffect>> StartupPersistentEffects;
	...
	// Grants StartupAbilities to the ASC (Server Only)
	void GrantStartupAbilities();

	// Applies all StartupPersistentEffects to Self (Server Only)
	void ApplyStartupPersistentEffects();
