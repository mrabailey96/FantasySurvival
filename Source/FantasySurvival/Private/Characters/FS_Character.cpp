...
AFS_Character::AFS_Character()
{
	...
	CameraComponent->bUsePawnControlRotation = false; // boom handles rotation
...
void AFS_Character::BeginPlay()
{
	...
	if (IsLocallyControlled())
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle, [this]()
			{
				if (const AFS_PlayerState* PlayerState = GetPlayerState<AFS_PlayerState>())
				{
...
void AFS_Character::PossessedBy(AController* NewController)
{
	...
	// Grant startup abilities on the server for testing
	GrantStartupAbilities();

	ApplyStartupPersistentEffects();
}

void AFS_Character::GrantStartupAbilities()
{
	...
	for (const TSubclassOf<UGameplayAbility>& GAClass : StartupAbilities)
	{
		if (*GAClass)
		{
			AbilitySystemComponent->GiveAbility(FGameplayAbilitySpec(GAClass, 1, INDEX_NONE, this));
		}
	}
}

void AFS_Character::ApplyStartupPersistentEffects()
{
	if (!HasAuthority() || !AbilitySystemComponent) return;

	for (const TSubclassOf<UGameplayEffect>& GEClass : StartupPersistentEffects)
	{
		if (*GEClass)
		{
...
void AFS_Character::InitializeAbilitySystem()
{
...
void AFS_Character::ApplyDefaultAttributes()
{
